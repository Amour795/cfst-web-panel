...
async function loadCfApiConfig() {
    try {
        ...
        if (data) {
            cfZoneId.value = data.zoneId || ''; cfDomain.value = data.domain || '';
            cfToken.value = data.token || ''; cfEmail.value = data.email || '';
...
function updateProgressUI(payload) {
    ...
    statusTag.innerText = payload.phase.includes('ping') ? 'PING' : payload.phase.includes('下载') ? 'DOWN' : 'RUN';
    ...
    if (payload.state === 'done' || payload.state === 'error') loadingSpinner.style.display = 'none';
}

function extractAndUpdateInput(text) {
    const matcher = allowCnameInput.checked ? mixedRegex : ipRegexGlobal;
    parsedTargets = [...new Set(String(text || '').match(matcher) || [])];
    ipCount.innerText = parsedTargets.length;
    ipInput.value = allowCnameInput.checked && parsedTargets.length === 0 ? text : parsedTargets.join('\n');
}
...
fetchSourceBtn.addEventListener('click', async () => {
    ...
    fetchSourceBtn.disabled = true; fetchSourceBtn.innerText = '拉取中...';
    try {
    ...
    } finally { fetchSourceBtn.disabled = false; fetchSourceBtn.innerText = '🌐 拉取源'; }
});
...
fileInput.addEventListener('change', (e) => {
    ...
    reader.readAsText(e.target.files[0]); fileInput.value = '';
});
...
function renderTable(dataArray, emptyMsg) {
    ...
    dataArray.forEach((item) => {
        ...
        tr.innerHTML = `
            <td class="text-center"><input type="checkbox" class="ip-checkbox" data-ip="${item.ip}" data-region="${item.region||''}"></td>
    ...
    document.querySelectorAll('.copyable-ip').forEach(el => el.addEventListener('click', () => copyToClipboard(el.dataset.ip, `✅ 已复制 ${el.dataset.ip}`)));
}
...
function updateSelectionState() {
    ...
    copySelectedBtn.disabled = !hasSel; saveSelectedBtn.disabled = !hasSel;
    tagSelectedBtn.disabled = !hasSel; deleteSelectedBtn.disabled = !hasSel;
    syncCfBtn.disabled = !hasSel;
    selectAllCheckbox.checked = hasSel && checked === cbs.length;
}
selectAllCheckbox.addEventListener('change', (e) => { document.querySelectorAll('.ip-checkbox').forEach(cb => cb.checked = e.target.checked); updateSelectionState(); });
copySelectedBtn.addEventListener('click', () => {
    const ips = Array.from(document.querySelectorAll('.ip-checkbox:checked')).map(cb => cb.dataset.ip).join('\n');
    copyToClipboard(ips, `✅ 成功复制`);
});
...
startBtn.addEventListener('click', async () => {
    ...
    progressFill.style.width = '0%'; progressLabel.innerText = '0%'; loadingSpinner.style.display = 'block';
    ...
    try {
        ...
        const response = await fetch('/api/start-test', { 
            ...
            body: JSON.stringify({
                ...
                runtimeOptions: {
                    incremental: incrementalMode.checked, incrementalDownOnly: incrementalDownOnly.checked,
        ...
        if (json.success) {
            ...
            statusTitle.innerText = '完成'; loadingSpinner.style.display = 'none';
...
async function loadCfstConfig() {
    try {
        ...
        if (json.success) {
            const c = json.data; cfstMode.value = c.mode||'tcp'; cfstUrlInput.value = c.url||'';
            cfstDtInput.value = c.dt||''; cfstDnInput.value = c.dn||''; cfstNInput.value = c.n||'';
            cfstTpInput.value = c.tp||''; cfstTopNInput.value = c.topN||'';
